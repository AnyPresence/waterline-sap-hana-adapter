 ...
 */
function run(query, model, table, client, cb){
	var _isDMLOperation = function(query){
		return (query.indexOf("INSERT") >= 0) || (query.indexOf("UPDATE") >= 0);
	};
	client.exec(query, function (err, results) {
		if (err) return cb(new Error(err.message), null);
		if(_isDMLOperation(query)){
			var getIdQuery = "SELECT * FROM " + table + " ORDER BY " + model.primaryKey + " DESC LIMIT 1";
			client.exec(getIdQuery, function(error, res){
				if (err) return cb(new Error(error.message), null);
				cb(null, transformToWaterlineModel(res, model));
			});
		} else {
			cb(null, transformToWaterlineModel(results, model));
		}
	});
}

function transformToWaterlineModel(results, model){
